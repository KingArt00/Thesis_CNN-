...
import utils
from pet_gan import PetGan_Attack
from data_utils import data_prefetcher, _transforms_catsdogs
from ensemble_model import transfer_init, ensemble_model
...
def main():
    ...
    train_data = dset.ImageFolder(
        root=FLAGS.data_dir, transform=train_transform)
    assert train_data
    ...
    if FLAGS.train_single:
        ...
        model, param_to_train = transfer_init(
            model, FLAGS.model, FLAGS.classes)
        model.to(device)
        ...
        for epoch in range(25):
            ...
            test_correct = 100 * test_correct / \
                (1-FLAGS.data_split) / num_train
            print('Eval loss: {:.4f}, accuracy: {:.4f}'.format(
                test_loss, test_correct))
            if (test_correct > best_acc):
                best_acc = test_correct
                torch.save(model, os.path.join(
                    FLAGS.model_dir, '{}.pth'.format(FLAGS.model)))
        print('Best test accuracy for model {}: {:.4f}'.format(
            FLAGS.model, best_acc))
    elif FLAGS.train_ensemble:
        print('Loading model...\n')
        model_names = ['mobilenet_v2', 'resnet18', 'densenet121',
                       'googlenet', 'resnext50_32x4d']
        model = ensemble_model(model_names, FLAGS.classes, FLAGS.model_dir)
        model.to(device)
    ...
    model.eval()
    pet_gan = PetGan_Attack(device, model, FLAGS.classes,
                           FLAGS.channels, 0, 1, FLAGS.model_dir)
    # pet_gan.netG = torch.load(os.path.join(FLAGS.model_dir, 'netG_epoch_{}.pth'.format(FLAGS.pretrained_epoch)))

    pet_gan.train(train_loader, FLAGS.epochs)
    ...
    with torch.no_grad():
        ...
        while data is not None:
            ...
            perturbed_data = pet_gan.adv_example(data)
            ...
            if len(adv_examples) < 64 and not (final_pred == target).all():
                indices = torch.ne(final_pred.ne(
                    target), init_pred.ne(target)).nonzero()
                for i in range(indices.shape[0]):
                    adv_ex = perturbed_data[indices[i]
                                            ].squeeze().detach().cpu().numpy()
                    adv_examples.append(
                        (init_pred[indices[i]].item(), final_pred[indices[i]].item(), adv_ex))
    ...
    if True:
        ...
        vutils.save_image(viz_sample, 'adv_examples.png',
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_dir', type=str,
                        default='./models', help='directory for trained models')
    parser.add_argument('--data_dir', type=str,
                        default='./cats-dogs-kaggle', help='Directory for dataset.')
    parser.add_argument('--data_split', type=float, default=0.8,
                        help='split ratio for train and val data')
    parser.add_argument('--cutout', type=boolean_string,
                        default=False, help='use cutout')
    parser.add_argument('--cutout_length', type=int,
                        default=64, help='number of epochs')
    ...
    parser.add_argument('--pretrained_epoch', type=int,
                        default=60, help='epoch number of pretrained generator')
