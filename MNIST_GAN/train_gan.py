...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Training MNIST GAN')
    parser.add_argument('--model', type=str, default="Model_X", required=False, choices=["Model_A", "Model_B", "Model_X"], help='model name (default: Model_X)')
    ...
    if target in range(0, 10):
        # bool variable to indicate targeted or untargeted attack
        is_targeted = True 
    ...
    # loss for fooling target model
    criterion_adv =  CWLoss 
    # for gan loss
    criterion_gan = nn.MSELoss() 
    #Hyperparameters
    # gan loss multiplication factor
    alpha = 1 
    # for hinge loss
    beta = 1 
    # number of generator updates for 1 discriminator update
    num_steps = 3 
    # perturbation bound, used in loss_hinge
    thres = c = 0.3 


    #Change to GPU and enable cuda
    device = 'cpu' 
